...
from libs.ocrTextDialog import OCRTextDialog
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def new_shape(self):
        ...
        if text is not None:
            ...
            # NEW: Ask for OCR ground truth
            ocr_dialog = OCRTextDialog(self)
            ocr_text = ocr_dialog.pop_up()

            shape = self.canvas.set_last_label(text, generate_color, generate_color)

            # NEW: Save OCR ground truth to shape
            shape.ocr_text = ocr_text  # <-- make sure Shape class supports this
            print("shape.ocr_text = ", ocr_text)
            self.add_label(shape)
        ...
        # # NEW: OCR GTRUTH
        # text = self.label_dialog.pop_up(ocr_text) #(item.text())
        # if text is not None:
        #     # NEW: Ask for OCR ground truth update
        #     current_shape = self.canvas.get_shape_from_item(item)
        #     current_ocr_text = getattr(current_shape, "ocr_text", "")
        #     ocr_dialog = OCRTextDialog(self, text=current_ocr_text)
        #     new_ocr_text = ocr_dialog.pop_up()
        ...
        #     # Update OCR ground truth in shape
        #     current_shape.ocr_text = new_ocr_text 
